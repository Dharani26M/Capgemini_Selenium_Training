...
public class RequirementTest extends BaseForOrangeHRM {
	...
	public void RequirementVacancy() throws InterruptedException{
		...
		wait.until(ExpectedConditions.elementToBeClickable(driver.findElement(By.xpath("//button[.=' Add ']")))).click();
		driver.findElement(By.xpath("//label[.='Vacancy Name']/../../child::div/following-sibling::div//input"))
		.sendKeys("CxxxD26");
		WebElement select= driver.findElement(By.xpath("//i[@class='oxd-icon bi-caret-down-fill oxd-select-text--arrow']"));
		select.click();
		WebElement w= wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//div[@role='listbox']//span[contains(.,'Account Assistant')]")));
		w.click();
		driver.findElement(By.cssSelector("[placeholder='Type description here']"))
		.sendKeys("please accept");
		driver.findElement(By.cssSelector("[placeholder='Type for hints...']")).click();
		Actions al=new Actions(driver);
		al.sendKeys("John ").pause(2000).sendKeys(Keys.ARROW_DOWN,Keys.ENTER).perform();
